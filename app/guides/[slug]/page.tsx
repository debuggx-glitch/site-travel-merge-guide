...
export default async function GuidePage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="journal-entry">
    <GuideStructuredData guide={guide}/>
    <nav className="crumbs" aria-label="Breadcrumb"><Link href="/">Travel journal</Link><span>✦</span><span>{guide.category}</span></nav>
    <header className="entry-heading"><p className="issue">Entry dated {guide.updated}</p><h1>{guide.title}</h1><p>{guide.description}</p></header>
    <section className="pocket-answer"><span>Keep this card</span><p>{guide.quickAnswer}</p></section>
    <GuideAd/>
    ...
    <div className="route-line">{guide.sections.map((section,index)=><Fragment key={section.heading}><section id={`stop-${index+1}`}><aside><span>{String(index+1).padStart(2,"0")}</span><i/></aside><div><p className="stop-label">Route stop {String(index+1).padStart(2,"0")}</p><h2>{section.heading}</h2>{section.paragraphs.map((paragraph)=><p key={paragraph}>{paragraph}</p>)}{section.visual&&section.visual.assetId!==leadVisual?.assetId?<figure data-asset-id={section.visual.assetId}><Image src={section.visual.src} width={section.visual.width} height={section.visual.height} sizes="(max-width: 900px) 100vw, 760px" alt={section.visual.alt}/><figcaption>{section.visual.caption}</figcaption></figure>:null}</div></section></Fragment>)}<section className="sources"><aside><span>✦</span></aside><div><p className="stop-label">Travel references</p><h2>Sources</h2>{guide.sources.map((source)=><p key={source.url}><a href={source.url} rel="noreferrer">{source.label} ↗</a></p>)}</div></section></div>
  </main>;
}
